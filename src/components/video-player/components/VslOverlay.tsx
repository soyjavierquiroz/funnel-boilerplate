...
export function VslOverlay({ onUnmute }: VslOverlayProps) {
  const primaryColor = 'rgb(var(--color-primary) / 1)';
  ...
  return (
    ...
    >
      ...
      >
        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-white/5" style={{ color: primaryColor }}>
          <Volume2 className="h-4 w-4 animate-pulse" fill="currentColor" />
        </span>
        <span className="text-[11px] font-light uppercase tracking-[0.14em] sm:text-xs" style={{ color: primaryColor }}>
