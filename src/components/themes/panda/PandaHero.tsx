...
import { KurukinPlayer as ThemeVideoPlayer } from './components/video-player/src/kurukin-video-player';

export function PandaHero() {
  const { enabled: isVideoEnabled, ...videoProps } = {
    ...funnelConfig.media.heroVideo,
    videoId: DNA.vslVideoId,
  };
  const primaryOffer = funnelConfig.pricing.products[funnelConfig.pricing.defaultProductKey];
  const ctaHref = primaryOffer?.checkoutUrl === '#checkout' ? '#pricing' : primaryOffer?.checkoutUrl || '#pricing';
  ...
  return (
    <section className="relative isolate overflow-hidden pb-16 pt-24 md:pb-24 md:pt-32">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" id="video">
        <div className="flex flex-col items-center text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-border-subtle/25 bg-surface/50 px-4 py-2 text-[11px] font-medium uppercase tracking-[0.24em] text-text-muted backdrop-blur-xl sm:text-xs">
            ...
            {DNA.copy.productName} listo para VSLs y ofertas
          </span>
          ...
          <div className="mt-6 max-w-5xl space-y-4 md:mt-8 md:space-y-5">
            <h1 className="text-4xl font-bold tracking-tighter text-text-main sm:text-5xl md:text-7xl md:leading-[0.95]">
              Presenta {DNA.copy.productName} como una experiencia premium desde el primer segundo.
            </h1>
            <p className="mx-auto max-w-2xl text-base font-light leading-7 text-text-muted sm:text-lg md:text-xl">
              El video principal, el copy y el pricing nacen del DNA para que el clon salga limpio, coherente y listo
              para vender sin strings heredados ni precios obsoletos.
            </p>
          </div>

          <div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-sm text-text-muted">
            <span className="rounded-full border border-border-subtle/20 bg-surface/40 px-4 py-2">
              Desde ${DNA.prices.main}
            </span>
            <span className="rounded-full border border-border-subtle/20 bg-surface/40 px-4 py-2">
              Valor referencial ${DNA.prices.regularPrice}
            </span>
            <span className="rounded-full border border-border-subtle/20 bg-surface/40 px-4 py-2">
              Stack ${DNA.prices.totalValue}
            </span>
          </div>

          <div className="relative mt-10 w-full md:mt-16">
            <div
              aria-hidden="true"
          ...
          <div className="mt-8 flex justify-center md:mt-10">
            ...
            >
              Quiero activar {DNA.copy.productName}
            </a>
          </div>
        </div>
      </div>
